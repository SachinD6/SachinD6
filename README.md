# Sachin Chaudhary

Software engineer. I build products people actually use, and the tooling that makes coding agents less annoying.

[Portfolio](https://sxch.dev) · [X/Twitter](https://x.com/sxch0) · [LinkedIn](https://linkedin.com/in/sachinxduhan) · [Email](mailto:sachinduhan.work@gmail.com) · [npm](https://www.npmjs.com/~sxch)

## Selected work

**[br0x](https://github.com/SachinD6/br0x)**

A fast browser for Linux. Idle tabs release their web process, ads and trackers are refused before they load, and six search engines switch from the address bar. Rust, GTK4, WebKitGTK.

**[Pi Harness](https://github.com/SachinD6/pi-resume-harness)** — [npm](https://www.npmjs.com/package/pi-resume-harness) · [pi.dev](https://pi.dev/packages/pi-resume-harness)

Resume Claude Code, Cursor, and Codex sessions inside [Pi](https://pi.dev). Searchable picker, strict project-cwd matching, inert handoff. TypeScript, no Python.

**[FresherGo](https://freshergo.com)**

Verified entry-level tech jobs, aggregated from official company career pages and updated daily. Search, saved roles, and weekly alerts.

## Focus

Tools that make other developers' lives easier, and products that hold up under real users. Right now: agent session/context tooling, verified job data at scale, and a memory-light browser for Linux.

Based in India. Open to full-time software engineering roles.
